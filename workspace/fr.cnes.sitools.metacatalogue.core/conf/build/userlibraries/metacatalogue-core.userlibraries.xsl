...
<xsl:param name="ROOT_DIRECTORY" select="'D:/CNES-ULISSE-2.0-GIT'" />
...
<xsl:param name="PORT" select="'8182'" />
	...
	<xsl:message>param ROOT_DIRECTORY default value D:/CNES-ULISSE-2.0-GIT </xsl:message>
	...
	<project name="sitools-userlibraries">
		...
		<path>
			...
			<xsl:for-each select="./archive">
				<file>
				<xsl:attribute name="name"><xsl:value-of select="replace(@path,'D:/CNES-ULISSE-2.0-GIT', $ROOT_DIRECTORY)"/></xsl:attribute>
				</file>
			</xsl:for-each>
			</filelist>
		</path> 
	...
	<xsl:template match="archive">
		<archive>
		<xsl:attribute name="path" select="replace(@path,'D:/CNES-ULISSE-2.0-GIT', $ROOT_DIRECTORY)"/>
		</archive>
	</xsl:template>
