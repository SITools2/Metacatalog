...
<xsl:param name="PORT" select="'8182'" />
	...
	<project name="sitools-userlibraries">
		...
		<path>
			...
			<xsl:for-each select="./archive">
				<file>
				<xsl:attribute name="name"><xsl:value-of select="replace(@path,'D:/CNES-ULISSE-METACATALOGUE', $ROOT_DIRECTORY)"/></xsl:attribute>
				</file>
			</xsl:for-each>
			</filelist>
		</path> 
	...
	<xsl:template match="archive">
		<archive>
		<xsl:attribute name="path" select="replace(@path,'D:/CNES-ULISSE-METACATALOGUE', $ROOT_DIRECTORY)"/>
		</archive>
	</xsl:template>
