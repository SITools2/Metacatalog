			...
			<xsl:for-each select="//gmd:MD_LegalConstraints/*">
				<xsl:if
					test="(name(.)='gmd:useLimitation' or name(.)='gmd:otherConstraints') and 
						(contains(string(./*),'WMO Essential')
                        or contains(string(./*),'WMOEssential')
                        or contains(string(./*),'WMOAdditional') 
						or contains(string(./*),'WMO Additional'))">
...
<!-- 			<field name="keyword_to_test_add_column"> -->
<!-- 				<xsl:value-of select="string(.)" /> -->
<!-- 			</field> -->
<!-- 		</xsl:for-each> -->
	
	
<!-- 		<field name="FIXED_VALUE">FIXED_VALUE</field> -->
		...
		<xsl:for-each select="gmd:identificationInfo//gmd:MD_DataIdentification|gmd:identificationInfo/srv:SV_ServiceIdentification">
			...
			<xsl:for-each select="gmd:citation/gmd:CI_Citation">
				<xsl:for-each select="gmd:identifier/gmd:MD_Identifier/gmd:code/gco:CharacterString|gmd:identifier/gmd:RS_Identifier/gmd:code/gco:CharacterString">
		...
		<xsl:for-each select="gmd:distributionInfo/gmd:MD_Distribution">
			...
			<xsl:for-each select="gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource/gmd:linkage/gmd:URL">
				<!-- RESOURCE LOCATOR, PART B 1.4 -->
				<field name="Resource_locator">
					<xsl:value-of select="string(.)" />
				</field>
			</xsl:for-each>
...
<!-- 			</xsl:for-each> -->
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<field name="Resource_type">dataset</field>
			</xsl:otherwise>
		</xsl:choose>
