...
public class OpenwisGeometryTool {
   ...
   *           the exception
   */
  public Filter buildFilter(Query query, String bbox) throws Exception {
    Filter filter = null;
    Geometry geometry = getGeometry(bbox);
    if (geometry != null) {
      filter = new CachingWrapperFilter(buildSpatialFilter(query, geometry));
    }
   ...
   *           the exception
   */
  public void removeMetadata(String uuid, String sourceCore) throws Exception {
    logger.info("\tRemove spatial metadata: {}", uuid);
    lock.lock();
    try {
      getSpatialIndexWriter().delete(uuid, sourceCore);
    }
    finally {
      lock.unlock();
    }
   ...
   *           the exception
   */
  public void removeMetadataByCore(String coreName) throws Exception {
    logger.info("\tRemove spatial metadata: {}", coreName);
    lock.lock();
    try {
      getSpatialIndexWriter().deleteByCore(coreName);
    }
    finally {
      lock.unlock();
    }
   ...
   *           the exception
   */
  public void addMetadata(String uuid, String wktGeometry, String sourceCore) throws Exception {
    ...
    try {
      ...
      SpatialIndexWriter writer = getSpatialIndexWriter();
      if (writer.featureExists(uuid)) {
        writer.delete(uuid);
      }
      writer.index(uuid, geometry, sourceCore);
  ...
  private SpatialFilter buildSpatialFilter(Query query, Geometry geometry, Element xml) throws Exception {
    lock.lock();
    try {
   ...
   */
  @SuppressWarnings("rawtypes")
  private SpatialFilter buildSpatialFilter(Query query, Geometry geometry) throws Exception {
    lock.lock();
    try {
      SpatialIndex index = getSpatialIndexWriter().getIndex();
      FeatureSource featureSource = getSpatialIndexWriter().getFeatureSource();
      IntersectionFilter intersectionFilter = new IntersectionFilter(query, geometry, featureSource, index);
  ...
  public Filter buildFilter(Query query, Element filterExpr, String filterVersion) {
    ...
    try {
      ...
      logger.error(MessageFormat.format("Error when parsing spatial filter (version: {0}):{1}. Error is: {2}",
   ...
   *           the exception
   */
  private Geometry getGeometry(String bbox) throws Exception {
    if (bbox != null && !bbox.isEmpty()) {
      String[] boxCoord = bbox.split(",");
      ...
      coordinates[0] = new Coordinate(long1, lat1);
      coordinates[1] = new Coordinate(long1, lat2);
      coordinates[2] = new Coordinate(long2, lat2);
      coordinates[3] = new Coordinate(long1, lat2);
      coordinates[4] = new Coordinate(long1, lat1);
  ...
  private static Constructor<? extends SpatialFilter> constructor(Class<? extends SpatialFilter> clazz)
    ...
    return clazz.getConstructor(Query.class, Geometry.class, FeatureSource.class, SpatialIndex.class);
  }
