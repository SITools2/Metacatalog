...
public abstract class SpatialFilter extends Filter {
	...
	static {
		...
		builder.add(PostgisSpatial.SPATIAL_INDEX_COLUMN_GEOMETRY,
				Geometry.class, DefaultGeographicCRS.WGS84);
	...
	public DocIdSet getDocIdSet(final IndexReader reader) throws IOException {
		...
		new IndexSearcher(reader).search(_query, new Collector() {
			...
			public final void collect(int doc) {			  
				...
				try {
					...
					String key = document
							.get(PostgisSpatial.SPATIAL_INDEX_UUID_FIELD);
		...
		if (!matches.isEmpty()) {
			bits = applySpatialFilter(matches, docIndexLookup, bits);
		}
		return new DocIdBitSet(bits);
	}

	private BitSet applySpatialFilter(Set<FeatureId> matches,
			Map<FeatureId, Integer> docIndexLookup, BitSet bits)
			throws IOException {
		Id fidFilter = _filterFactory.id(matches);
		String ftn = _featureSource.getSchema().getName().getLocalPart();
		String[] geomAtt = { _featureSource.getSchema().getGeometryDescriptor()
				.getLocalName() };
		...
		try {
			...
			if (!warned) {
				...
				logger.warn(e.getMessage()
						+ " errors are occuring with filter: " + getFilter(), e);
			}
			logger.debug(MessageFormat.format(
					"{0}: occurred during a search: {1} on feature: {2}",
					e.getMessage(), getFilter(), feature.getDefaultGeometry()),
					e);
			return false;
		}
	}
	...
	/**
	 * Returns all the FeatureId and ID attributes based on the query against
	 * the spatial index
	 * 
	 * @return all the FeatureId and ID attributes based on the query against
	 *         the spatial index
	 */
	protected synchronized Map<String, FeatureId> unrefinedSpatialMatches() {
		if (_unrefinedMatches == null) {
			Geometry geom = null;
			// _index.query returns geometries that intersect with provided
			// envelope. To use later a spatial filter that
			// provides geometries that don't intersect with the query envelope
			// (_geom) should be used a full extent
			// envelope in this method, instead of the query envelope (_geom)
			if (getFilter().getClass().getName()
					.equals("org.geotools.filter.spatial.DisjointImpl")) {
				try {
					...
					logger.error(ex.getMessage()
							+ ": Error in unrefinedSpatialMatches.", ex);
			...
			List<Pair<FeatureId, String>> fids = _index.query(geom
					.getEnvelopeInternal());
	...
	protected org.opengis.filter.Filter createFilter(FeatureSource source) {
		...
		return createGeomFilter(_filterFactory, geomPropertyName,
				geomExpression);
	}
